...
public class CartService {
    ...
    // Gets the cart that is associated with the user by passing in their user id as
    // a parameter
    public Cart getCartFromUserId(String userId) {
        return cartDAO.getCartFromUserId(userId);
    }

    // Adds individual products into the cart, making sure to keep track of the
    // quantity to create cartproducts later
    ...
    // Removes a product from a cart by passing in the cart's id and the product id
    // of the item in question
    public void removeProductFromCart(String cartId, String productId) {
        cartDAO.removeProductFromCart(cartId, productId);
    }

    // Gives the user the option to change the amount of any particular item that
